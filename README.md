...
Built to study how multi-day performance data can refine algorithmic trading strategies through adaptive parameter tuning and group optimization.

- Overview

This system automates the full research cycle — from dataset collection to multi-day testing, ML-style optimization, and recursive group formation.
...
Each variation and group is evaluated using multiple 5-day performance metrics, stored in SQLite databases.
- PP (Profit Percentage): Primary profit metric for each day
- Median PP/Wilson Lower Bound: Emphasizes reliable returns with statistical confidence to ensure lower volatility
- MAD/IQR (Dispersion Metrics): Penalizes volatility if the values skews too much. One of my top metrics in my ML logic and score formulas
- Buy/Count CV (Coefficent of Variation): Analyzes daily buy count consistency
- Repeat Ticker Rate (RTR): Measures concentration of buys and overlap of same tickers bought across multiple days
- Recency Weighting/EWPP: Favors improvments in later dates, meaning better performance in Friday is favored over Monday
...
- System Flow Summary
Google Sheets DDs and CSV exporters and data scrapers -> Python (MP1-P1, Excel Macros, MP1-P2) loop -> SQLite Database sorting -> (MP1-P3) -> Threading Recursion Engine (MP2-P2) -> Top Group Executor (MP2-P3)

(MP0) Fetch all tickers using macro for selected date in Google Sheets (GS)
      ↓
(MP0) Send parts of ticker list to the DDs and copy the GoogleFinance formula for the data
      ↓
(MP0) Run macro that collects all the data and compiles it into CSV files all formatted
      ↓
(MP0) Upload tickers to dictionaries and datasets in SQLite tables after filtering
      ↓
(MP1 - P1 LOOP START) Pastes batches of the different datasets into Excel, and triggers the cycling macro
      ↓
(MP1 - P1) Cycling macro cycles the datasets and records data, using helper cells to keep track of progress and communicate with python
      ↓
(MP1 - P1) Saves data for 5 days and calculates metrics and saves them in the database
      ↓
(MP1 - P2) Processes all 5 day metrics and parameters in the tables, runs ML inspired logic to choose the next best combinations of values for each algorithm
      ↓
(MP1 - P2 LOOP END) Pastes the new 8 variation parameters for each of the 10 algorithms in Excel
      ↓
(MP1 - P3) Filters all variations tested after loop, and leaves only leaves a few hundred of the top variations per algo using complex scoring formulas that use various metrics
      ↓
(MP2 - P2) Uses pruned tables to then start testing possible group combinations with every variation used as a base anchor in their own thread
      ↓
(MP2 - P2) Compares new shared ticker list, and recalculates new 5 day metrics, and scores the current group to save it or not. Enforces hard gates to end recursion branches early if shows early undesirable results
      ↓
(MP2 - P2) Saves top groups for each size (3-6) including all metrics into the database along with their parameters
      ↓
(MP2 - P3) Tests the top performing group by exporting its parameters to Excel and running the new datasets for the latest day to collect shared buys for tomorrow

- Design Notes

- This entirely built on Notepad, and exectured via CMD by running the file (python mp1.py), without IDEs, auto-save, or version control.
...
- I developed it iteravely through trial, debugging, and optimization. One small change at a time, refinining it little by little. 
